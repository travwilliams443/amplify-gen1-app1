...
export default function AccountCardCollectionWithDate(props) {
    ...
    while ((newCache.length < cacheUntil || !isPaginated) && newNext != null) {
      ...
      const result = (
        await client.graphql({
          query: listAccounts.replaceAll("__typename", ""),
          variables,
        })
      ).data.listAccounts;
  ...
  return (
    <div>
      <Collection
        ...
        {...getOverrideProps(overrides, "AccountCardCollectionWithDate")}
        {...rest}
      >
        {(item, index) => {
          if (loading) {
